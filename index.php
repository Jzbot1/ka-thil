...
if ($setting_result && $setting_result->num_rows > 0) {
    ...
    if (!empty($row['store_logo']))
        $setting['store_logo'] = $row['store_logo'];
    if (!empty($row['fav_icon']))
        $setting['fav_icon'] = $row['fav_icon'];
    if (!empty($row['facebook']))
        $setting['facebook'] = $row['facebook'];
    if (!empty($row['instagram']))
        $setting['instagram'] = $row['instagram'];
    if (!empty($row['whatsapp']))
        $setting['whatsapp'] = $row['whatsapp'];
    if (!empty($row['whatsapp_group']))
        $setting['whatsapp_group'] = $row['whatsapp_group'];
    if (!empty($row['description']))
        $setting['description'] = $row['description'];
    if (!empty($row['keywords']))
        $setting['keywords'] = $row['keywords'];
    if (isset($row['is_banner_on']))
        $setting['is_banner_on'] = $row['is_banner_on'];
    if (isset($row['is_maintenance']))
        $setting['is_maintenance'] = $row['is_maintenance'];
    if (!empty($row['flash_sale_end']))
        $setting['flash_sale_end'] = $row['flash_sale_end'];
}
...
<head>
    ...
    <link
        href="https://fonts.googleapis.com/css2?family=Poppins:wght@300;400;500;600;700&family=DynaPuff:wght@400;600&display=swap"
        rel="stylesheet">
    <style>
        :root {
            --bg-deep: #08203E;
        ...
        .bg-blob {
            ...
            width: 500px;
            height: 500px;
            background: linear-gradient(135deg, rgba(8, 32, 62, 0.2) 0%, rgba(85, 124, 147, 0.2) 100%);
            filter: blur(80px);
            border-radius: 50%;
    ...
    <header class="fixed top-0 w-full z-40 glass-panel h-16">
        <div class="max-w-md mx-auto px-4 h-full flex items-center justify-between">
            <div class="flex items-center gap-2">
                ...
                <?php else: ?>
                    <a href="auth/login"
                        class="px-4 py-1.5 rounded-full bg-themeDark text-white text-xs font-bold shadow-lg">LOG IN</a>
                <?php endif; ?>
            </div>
            <div class="font-bold text-lg text-themeDark font-dynapuff tracking-wider">
                <?= htmlspecialchars($setting['store_name']); ?></div>
            <div class="flex items-center gap-2">
                <button id="pwa-install-btn"
                    class="hidden w-9 h-9 rounded-full bg-themeDark text-white flex items-center justify-center border border-white/30 shadow-lg animate-bounce">
    ...
    <main class="max-w-md mx-auto pt-20 px-3">
        <?php if ($setting['is_banner_on'] == 1 && !empty($banners)): ?>
            ...
            <div class="mb-6 relative group">
                <div
                    class="absolute inset-y-0 left-4 flex items-center pointer-events-none transition-colors group-focus-within:text-themeDark text-themeDark/40">
                ...
                <div id="searchBadge"
                    class="absolute right-4 top-1/2 -translate-y-1/2 px-2 py-1 bg-themeDark/10 rounded-lg text-[8px] font-black uppercase text-themeDark/40 tracking-tighter opacity-0 transition-opacity">
                    Press Enter
                </div>
            </div>
        <?php endif; ?>
        ...
        <div
            class="mb-6 bg-white/10 backdrop-blur-md border border-white/10 rounded-xl py-2.5 px-3 flex items-center gap-3 overflow-hidden shadow-sm">
            ...
            <div class="overflow-hidden w-full relative h-5">
                <p class="animate-scroll-text whitespace-nowrap absolute text-[11px] text-themeDark/80 font-bold top-0">
                    Welcome to <?= htmlspecialchars($setting['store_name']); ?> - Instant Topup, 100% Secure! ⚡ Best
                    Prices Guaranteed!
                </p>
            </div>
        </div>
        ...
        <div class="grid grid-cols-4 gap-2 mb-8">
            ...
            foreach ($links as $link):
                ...
                <a href="<?= $link['url'] ?>" class="flex flex-col items-center gap-2">
                    <div
                        class="w-12 h-12 rounded-2xl bg-white/10 border border-white/10 flex items-center justify-center shadow-lg transition hover:bg-white/20">
        ...
        if ($is_flash_active):
            ...
            <div class="mb-8">
                <h3 class="text-sm font-bold text-themeDark flex items-center justify-between gap-2 mb-4 px-1">
                    <div class="flex items-center gap-2">
                        <span
                            class="w-1 h-4 bg-orange-500 rounded-full shadow-[0_0_8px_rgba(249,115,22,0.5)] animate-pulse"></span>
                    ...
                    <?php if (!empty($setting['flash_sale_end'])): ?>
                        <div id="flash-sale-timer"
                            class="flex items-center gap-1 bg-white/10 border border-white/10 px-2 py-0.5 rounded-lg shadow-sm">
                    ...
                    <?php foreach ($flash_sales as $flash):
                        ...
                        <a href="product/<?= htmlspecialchars($flash['game_slug'] ?? '') ?>?auto_select=<?= urlencode($flash['product_id']) ?>"
                            ...
                            <div
                                ...
                                <?php if ($discount > 0): ?>
                                    <div
                                        class="absolute top-0 right-0 z-20 bg-themeDark text-white text-[6px] font-black px-1 py-0.5 rounded-bl-md uppercase tracking-tighter">
                                        -<?= $discount ?>%
                                    </div>
                                <?php endif; ?>
                                <img src="<?= $f_img ?>"
                            ...
                            <div class="flex flex-col items-center w-full px-0.5">
                                ...
                                <div class="flex flex-col items-center justify-center w-full mt-0.5">
                                    <span
                                        class="text-[9px] font-black text-themeDark leading-none">₹<?= number_format($flash['price'], 0) ?></span>
                                    <span
                                        class="text-[6px] text-themeDark/40 line-through leading-none mt-0.5">₹<?= number_format($flash['old_price'], 0) ?></span>
                                </div>
            ...
            if (!empty($game_data[$key])):
                ...
                <div class="mb-8">
                    <h3 class="text-sm font-bold text-themeDark flex items-center gap-2 mb-4 px-1">
                        <span
                            class="w-1 h-4 <?= htmlspecialchars($sec['color']) ?> rounded-full shadow-[0_0_8px_rgba(59,130,246,0.5)]"></span>
                                ...
                                <div
                                    ...
                                    <?php if ($is_out_of_stock): ?>
                                        <div class="absolute inset-0 bg-themeDark/80 flex items-center justify-center">
                                            <span
                                                class="text-[7px] text-white font-black bg-red-600 px-1.5 py-0.5 rounded uppercase">Sold
                                                Out</span>
                                        </div>
                                    <?php endif; ?>
                                </div>
        ...
        <div class="mb-8 mt-2">
            <h3 class="text-sm font-bold text-themeDark flex items-center justify-between gap-2 mb-4 px-1">
                ...
                <a href="<?= BASE_URL ?>/tutorial"
                    class="text-[10px] font-black text-white/40 hover:text-white transition flex items-center gap-1">
            ...
            <div class="grid grid-cols-3 gap-2 mb-3">
                ...
                <!-- Recharge Card -->
                <a href="<?= BASE_URL ?>/tutorial#recharge" onclick="sessionStorage.setItem('tutCat','recharge')"
                    ...
                    <div class="flex items-center gap-1 mt-auto">
                        <span class="text-[8px] font-black text-cyan-400 bg-cyan-400/10 px-1.5 py-0.5 rounded-full">6
                            steps</span>
                    </div>
                    <!-- Hover glow -->
                ...
                <!-- Wallet Card -->
                <a href="<?= BASE_URL ?>/tutorial#wallet" onclick="sessionStorage.setItem('tutCat','wallet')"
                    class="group relative overflow-hidden rounded-2xl border border-white/10 p-3.5 flex flex-col gap-2 transition active:scale-95"
                    style="background:rgba(255,255,255,0.08);backdrop-filter:blur(12px);">
                    <div class="w-9 h-9 rounded-xl flex items-center justify-center text-lg mb-0.5"
                    ...
                    <div class="flex items-center gap-1 mt-auto">
                        <span
                            class="text-[8px] font-black text-purple-400 bg-purple-400/10 px-1.5 py-0.5 rounded-full">4
                            steps</span>
                    </div>
                    <div class="absolute inset-0 rounded-2xl opacity-0 group-hover:opacity-100 transition pointer-events-none"
                ...
                <!-- Register Card -->
                <a href="<?= BASE_URL ?>/tutorial#register" onclick="sessionStorage.setItem('tutCat','register')"
                    class="group relative overflow-hidden rounded-2xl border border-white/10 p-3.5 flex flex-col gap-2 transition active:scale-95"
                    style="background:rgba(255,255,255,0.08);backdrop-filter:blur(12px);">
                    <div class="w-9 h-9 rounded-xl flex items-center justify-center text-lg mb-0.5"
                    ...
                    <div class="flex items-center gap-1 mt-auto">
                        <span
                            class="text-[8px] font-black text-emerald-400 bg-emerald-400/10 px-1.5 py-0.5 rounded-full">4
                            steps</span>
                    </div>
                    <div class="absolute inset-0 rounded-2xl opacity-0 group-hover:opacity-100 transition pointer-events-none"
