...
<body class="pb-32">
    <header class="fixed top-0 w-full z-50 bg-black/20 backdrop-blur-xl h-16 border-b border-white/10">
        <div class="max-w-md mx-auto px-5 h-full flex items-center justify-between">
            ...
            <div class="font-bold text-lg text-white font-dynapuff">News & Updates</div>
            <div class="w-10"></div>
        </div>
    </header>
    ...
    <main class="max-w-md mx-auto px-4 mt-20 space-y-6">
        ...
        <?php if($notifications->num_rows == 0): ?>
            <div class="py-20 text-center glass-panel rounded-[2rem]">
                <i class="fa-solid fa-bell-slash text-4xl text-white/20 mb-4"></i>
                <p class="text-sm font-bold text-white/40 uppercase tracking-widest">No notifications yet</p>
            </div>
        <?php endif; ?>

        <?php while($n = $notifications->fetch_assoc()): ?>
            <div class="glass-panel p-6 rounded-[2.5rem] noti-card border border-white/10 relative overflow-hidden">
                <div class="flex items-center gap-3 mb-4">
                    <div class="w-10 h-10 rounded-2xl bg-blue-500/10 flex items-center justify-center text-blue-400">
                        <i class="fa-solid fa-bullhorn"></i>
                    </div>
                    <div>
                        <h3 class="font-black text-white leading-tight"><?= htmlspecialchars($n['title']) ?></h3>
                        <p class="text-[9px] font-bold text-white/40 uppercase tracking-widest"><?= date('d M, Y', strtotime($n['created_at'])) ?></p>
                    </div>
                </div>
                ...
                ?>
                    <div class="rounded-3xl overflow-hidden mb-4 shadow-xl shadow-black/20 aspect-video">
                        <iframe src="<?= $embed ?>" class="w-full h-full" frameborder="0" allowfullscreen></iframe>
                    </div>
                <?php elseif(!empty($n['image_url'])): ?>
                    <div class="rounded-3xl overflow-hidden mb-4 shadow-xl shadow-black/20">
                ...
                <div class="flex items-center justify-between mt-6 pt-4 border-t border-white/10">
                    <span class="px-3 py-1 bg-white/10 rounded-full text-[8px] font-black uppercase text-white/40 tracking-widest">System Update</span>
                    <button class="text-[10px] font-black text-blue-400 uppercase tracking-widest">Read More <i class="fa-solid fa-chevron-right ml-1"></i></button>
                </div>
            </div>
        <?php endwhile; ?>
