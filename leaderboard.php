    ...
    <main class="max-w-md mx-auto px-4 mt-20">
        ...
        <div class="glass-panel rounded-3xl p-4 mb-8 flex items-center justify-between">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <p class="text-[10px] font-bold text-white/40 uppercase tracking-widest leading-none">Resets In</p>
                    <p id="countdown" class="text-sm font-black text-white mt-1">Calculating...</p>
                </div>
            </div>
        ...
        <div class="flex items-end justify-center gap-4 mb-10 mt-14">
            <?php if(count($leaderboard) >= 2): ?>
                ...
                <div class="podium-item flex flex-col items-center">
                    ...
                    <p class="text-[10px] font-black text-white truncate w-20 text-center"><?= htmlspecialchars($leaderboard[1]['username']) ?></p>
                    <p class="text-[9px] font-bold text-white/60 mt-1">₹<?= number_format($leaderboard[1]['total_spent'], 0) ?></p>
                </div>
            <?php endif; ?>
            ...
            <?php if(count($leaderboard) >= 1): ?>
                ...
                <div class="podium-item flex flex-col items-center -mt-6">
                    ...
                    <p class="text-xs font-black text-white truncate w-24 text-center"><?= htmlspecialchars($leaderboard[0]['username']) ?></p>
            ...
            <?php if(count($leaderboard) >= 3): ?>
                ...
                <div class="podium-item flex flex-col items-center">
                    ...
                    <p class="text-[10px] font-black text-white truncate w-20 text-center"><?= htmlspecialchars($leaderboard[2]['username']) ?></p>
                    <p class="text-[9px] font-bold text-white/60 mt-1">₹<?= number_format($leaderboard[2]['total_spent'], 0) ?></p>
                </div>
            <?php endif; ?>
        </div>
        ...
        <div class="space-y-3 mb-10">
            <?php for($i=3; $i < count($leaderboard); $i++): ?>
                <div class="glass-panel p-4 rounded-3xl flex items-center justify-between animate-slide" style="animation-delay: <?= $i*0.1 ?>s">
                    <div class="flex items-center gap-4">
                        <div class="w-8 h-8 rounded-xl bg-white/10 flex items-center justify-center text-xs font-black text-white"><?= $i + 1 ?></div>
                        <div class="relative">
                        ...
                        <div>
                            <p class="text-xs font-black text-white"><?= htmlspecialchars($leaderboard[$i]['username']) ?></p>
                            <p class="text-[9px] font-bold text-white/40 uppercase">Elite Member</p>
                        </div>
                    </div>
                    <div class="text-right">
                        <p class="text-sm font-black text-white">₹<?= number_format($leaderboard[$i]['total_spent'], 0) ?></p>
                    </div>
                </div>
            <?php endfor; ?>
            ...
            <?php if(count($leaderboard) == 0): ?>
                <div class="py-20 text-center glass-panel rounded-[2rem]">
                    <i class="fa-solid fa-trophy text-4xl text-white/10 mb-4"></i>
                    <p class="text-sm font-bold text-white/40 uppercase tracking-widest">No rankings yet</p>
                </div>
            <?php endif; ?>
        </div>
